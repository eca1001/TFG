...
def inicial(): #solo es necesario si no se tiene guardado el grafo
    ...
    for a in range(len(public)):
        nodoEnt = public.to_numpy()[a][26]
    
        if str(nodoEnt) != "nan":
            listaEntorno = nodoEnt.split(';')

            for e in itertools.combinations(listaEntorno,2):
    ...
    #Guardar grafo
    nx.write_graphml(G, "grafosTecnologias/grafoHerramientas.graphml")
    return G
...
def leerGrafo():
    #Cargar grafo
    G = nx.read_graphml("grafosTecnologias/grafoHerramientas.graphml")
    return G
...
def gradosNodos(G, umbral):
    ...
    plt.title('Porcentaje de relación de la herramienta \n', fontsize = 20)
    plt.axis("equal")
    plt.savefig("static/images/graficosSectores/GraficoSectoresHerramientas.jpg")
...
def propiedadesRed(G, umbral):
    ...
    plt.savefig("static/images/histogramas/histogramaHerramientas.jpg")
...
def grafoInteractivo(G, umbral):
    ...
    #Choose a title!
    title = 'Red de Relación de Herramientas'
...
def vecinos(G, umbral):
    libro = xlsxwriter.Workbook('excel/vecinos/vecinosHerramientas.xlsx')
    ...
    libro.close()
    if ma==3:
        df = pd.read_excel("excel/vecinos/vecinosHerramientas.xlsx", usecols=("A:D"))
    elif ma==2:
        df = pd.read_excel("excel/vecinos/vecinosHerramientas.xlsx", usecols=("A:C"))
    else:
        df = pd.read_excel("excel/vecinos/vecinosHerramientas.xlsx", usecols=("A:B"))

    df.fillna('', inplace=True)
    df.to_html('static/images/tablas/tablaVecinosHerramientas.html', justify='center', col_space=100, table_id="myTable")

def crearTablaFiltro():
    a = open('static/images/tablas/tablaVecinosHerramientas.html','r')
    f = open('static/images/tablaFiltro/tablaConFiltroHerramientas.html','w')
    mensaje1="""
    <html>
    <head>
    ...
    * {
    ...
    <h2><b><center>RECOMENDADOR DE HERRAMIENTAS</center></b></h2>

    <input type="text" id="myInput" onkeyup="myFunction()" placeholder="Filtrar por herramienta..." title="Type in a name">
    """
    f.write(mensaje1)
...
def nodosRestantes(G, H):
    nF = G.nodes()-H.nodes()
    libro = xlsxwriter.Workbook('excel/faltantes/faltantesHerramientas.xlsx')
    hoja = libro.add_worksheet()
    ...
    hoja.write(0, 0, "Herramientas No Conectadas")
    
    #Cerramos el libro
    libro.close()
    
    df = pd.read_excel("excel/faltantes/faltantesHerramientas.xlsx")

    df.fillna('', inplace=True)
    df.to_html('static/images/tablas/tablaHerramientasFaltantes.html', justify='center', col_space=100, table_id="myTable")

def crearTablaFiltroRestantes():
    a = open('static/images/tablas/tablaHerramientasFaltantes.html','r')
    f = open('static/images/tablaFiltro/tablaConFiltroRestantesHerramientas.html','w')
    mensaje1="""
    <html>
    <head>
    <meta name="viewport" content="width=device-width, initial-scale=1">
