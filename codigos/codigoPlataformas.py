...
def inicial(): #solo es necesario si no se tiene guardado el grafo
    ...
    for a in range(len(public)):
        nodoPlat = public.to_numpy()[a][20]
    
        if str(nodoPlat) != "nan":
            listaPlat = nodoPlat.split(';')

            for e in itertools.combinations(listaPlat,2):
    ...
    #Guardar grafo
    nx.write_graphml(G, "grafosTecnologias/grafoPlataformas.graphml")
    return G
...
def leerGrafo():
    #Cargar grafo
    G = nx.read_graphml("grafosTecnologias/grafoPlataformas.graphml")
    return G
...
def gradosNodos(G, umbral):
    ...
    plt.title('Porcentaje de relación de la plataforma \n', fontsize = 20)
    plt.axis("equal")
    plt.savefig("static/images/graficosSectores/GraficoSectoresPlat.jpg")
...
def propiedadesRed(G, umbral):
    ...
    plt.savefig("static/images/histogramas/histogramaPlat.jpg")
...
def grafoInteractivo(G, umbral):
    ...
    #Choose a title!
    title = 'Red de Relación de Plataformas'
...
def vecinos(G, umbral):
    libro = xlsxwriter.Workbook('excel/vecinos/vecinosPlat.xlsx')
    ...
    libro.close()
    if ma==3:
        df = pd.read_excel("excel/vecinos/vecinosPlat.xlsx", usecols=("A:D"))
    elif ma==2:
        df = pd.read_excel("excel/vecinos/vecinosPlat.xlsx", usecols=("A:C"))
    else:
        df = pd.read_excel("excel/vecinos/vecinosPlat.xlsx", usecols=("A:B"))

    df.fillna('', inplace=True)
    df.to_html('static/images/tablas/tablaVecinosPlat.html', justify='center', col_space=100, table_id="myTable")

def crearTablaFiltro():
    a = open('static/images/tablas/tablaVecinosPlat.html','r')
    f = open('static/images/tablaFiltro/tablaConFiltroPlat.html','w')
    mensaje1="""
    <html>
    <head>
    ...
    * {
    ...
    <h2><b><center>RECOMENDADOR DE PLATAFORMAS</center></b></h2>

    <input type="text" id="myInput" onkeyup="myFunction()" placeholder="Filtrar por plataforma..." title="Type in a name">
    """
    f.write(mensaje1)
...
def nodosRestantes(G, H):
    nF = G.nodes()-H.nodes()
    libro = xlsxwriter.Workbook('excel/faltantes/faltantesPlat.xlsx')
    hoja = libro.add_worksheet()
    ...
    hoja.write(0, 0, "Plataformas No Conectados")
    
    #Cerramos el libro
    libro.close()
    
    df = pd.read_excel("excel/faltantes/faltantesPlat.xlsx")

    df.fillna('', inplace=True)
    df.to_html('static/images/tablas/tablaPlatFaltantes.html', justify='center', col_space=100, table_id="myTable")

def crearTablaFiltroRestantes():
    a = open('static/images/tablas/tablaPlatFaltantes.html','r')
    f = open('static/images/tablaFiltro/tablaConFiltroRestantesPlat.html','w')
    mensaje1="""
    <html>
    <head>
    <meta name="viewport" content="width=device-width, initial-scale=1">
