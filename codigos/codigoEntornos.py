...
def inicial(): #solo es necesario si no se tiene guardado el grafo
    ...
    for a in range(len(public)):
        nodoEntornos = public.to_numpy()[a][28]
    
        if str(nodoEntornos) != "nan":
            listaEntornos = nodoEntornos.split(';')

            for e in itertools.combinations(listaEntornos,2):
    ...
    #Guardar grafo
    nx.write_graphml(G, "grafosTecnologias/grafoEntornos.graphml")
    return G
...
def leerGrafo():
    #Cargar grafo
    G = nx.read_graphml("grafosTecnologias/grafoEntornos.graphml")
    return G
...
def gradosNodos(G):
    ...
    plt.title('Porcentaje de relación de los marcos de trabajo \n', fontsize = 20)
    plt.axis("equal")
    plt.savefig("static/images/graficosSectores/GraficoSectoresEntornos.jpg")
...
def propiedadesRed(G):
    ...
    plt.savefig("static/images/histogramas/histogramaEntornos.jpg")
...
def grafoInteractivo(G):
    ...
    #Choose a title!
    title = 'Red de Relación de Entornos'
...
def vecinos(G):
    libro = xlsxwriter.Workbook('excel/vecinos/vecinosEntornos.xlsx')
    ...
    libro.close()
    if ma==3:
        df = pd.read_excel("excel/vecinos/vecinosEntornos.xlsx", usecols=("A:D"))
    elif ma==2:
        df = pd.read_excel("excel/vecinos/vecinosEntornos.xlsx", usecols=("A:C"))
    else:
        df = pd.read_excel("excel/vecinos/vecinosEntornos.xlsx", usecols=("A:B"))

    df.fillna('', inplace=True)
    df.to_html('static/images/tablas/tablaVecinosEntornos.html', justify='center', col_space=100, table_id="myTable")

def crearTablaFiltro():
    a = open('static/images/tablas/tablaVecinosEntornos.html','r')
    f = open('static/images/tablaFiltro/tablaConFiltroEntornos.html','w')
    mensaje1="""
    <html>
    <head>
    ...
    * {
    ...
    <h2><b><center>RECOMENDADOR DE ENTORNOS</center></b></h2>

    <input type="text" id="myInput" onkeyup="myFunction()" placeholder="Filtrar por entornos..." title="Type in a name">
    """
    f.write(mensaje1)
...
def nodosRestantes(G, H):
    nF = G.nodes()-H.nodes()
    libro = xlsxwriter.Workbook('excel/faltantes/faltantesEnt.xlsx')
    hoja = libro.add_worksheet()
    ...
    hoja.write(0, 0, "Entornos No Conectados")
    
    #Cerramos el libro
    libro.close()
    
    df = pd.read_excel("excel/faltantes/faltantesEnt.xlsx")

    df.fillna('', inplace=True)
    df.to_html('static/images/tablas/tablaEntornosFaltantes.html', justify='center', col_space=100, table_id="myTable")

def crearTablaFiltroRestantes():
    a = open('static/images/tablas/tablaEntornosFaltantes.html','r')
    f = open('static/images/tablaFiltro/tablaConFiltroRestantesEntornos.html','w')
    mensaje1="""
    <html>
    <head>
    <meta name="viewport" content="width=device-width, initial-scale=1">
