...
def inicial(): #solo es necesario si no se tiene guardado el grafo
    ...
    #Guardar grafo
    nx.write_graphml(G, "grafosTecnologias/grafoLenguajes.graphml")
    return G
...
def leerGrafo():
    #Cargar grafo
    G = nx.read_graphml("grafosTecnologias/grafoLenguajes.graphml")
    return G
...
def vecinos(G, umbral):
    ...
    libro.close()
    if ma==3:
        df = pd.read_excel("excel/vecinos/vecinosLen.xlsx", usecols=("A:D"))
    elif ma==2:
        df = pd.read_excel("excel/vecinos/vecinosLen.xlsx", usecols=("A:C"))
    else:
        df = pd.read_excel("excel/vecinos/vecinosLen.xlsx", usecols=("A:B"))
...
def nodosRestantes(G, H):
    nF = G.nodes()-H.nodes()
    libro = xlsxwriter.Workbook('excel/faltantes/faltantesLen.xlsx')
    hoja = libro.add_worksheet()
    ...
    df = pd.read_excel("excel/faltantes/faltantesLen.xlsx")
